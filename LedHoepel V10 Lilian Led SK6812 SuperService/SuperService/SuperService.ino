...
//------------------------------LED------------------------------------//
const unsigned int ledsPerStrip     =       560;                              // 4x140 rgb leds =  560 leds per Pin
const unsigned int RGBWByteLength   =         6;                                // 1 Int is 4 Bytes. 6 Ints is 24 Bytes is RGB. 8 Ints is 32 Bytes is RGB
...
// ----------------- NETWORK -----------------------------//
byte mac[]     = { 0x90, 0xA2, 0xDA, 0x0D, 0x4C, 0x82}  ; // the mac adress in HEX of ethernet module/shield
byte ip[]      = {  192,  168,    0,    8};               // the IP adress of your device, that should be in same universe of the network you are using
...
const byte first_universe_number     = 64;                  // CHANGE FOR YOUR SETUP most software this is 1, some software send out artnet first universe as zero.
const byte universeSize              = 32;                 // used for doublecheking if given universe in the artnetpacket  should be dealt with, or not
const byte max_universe = first_universe_number + universeSize; // don't touch this 
const unsigned int number_of_channels      = 420;                  // number of channels used per universe
...
const unsigned int MAX_BUFFER_UDP          = 768;                  // leave as is , this is big enough to get artnet packets
...
const byte start_address             = 0;                  // 0 if you want to read from channel 1
const byte art_net_header_size      = 17;                  // we skip these bytes in the data poacket, to get to the universe and channels
const byte baseOffset               = art_net_header_size + 1; // preCompute the baseOffset, don't touch this
int sequenceCount                   = 0;                  // we dont used this , but this is to get the right order of packets
// -------------------------------------------------------//

const byte strips = 8;
const unsigned int pixelCount = ledsPerStrip * strips;
...
*/
  ...
  if (incoming_universe < first_universe_number || incoming_universe > max_universe || incoming_universe == 6454) {
    return;
  }
  ...
  //int baseOffset = art_net_header_size + 1;
  for (uint8_t i = 0; i < number_of_channels; i++) {
      channel_buffer[i] = byte(packetBuffer[i + baseOffset]);
  }
  ...
  unsigned int baseIndex = (incoming_universe - first_universe_number) * number_of_channels;
  for (unsigned i = 0; i < number_of_channels; i++) {
      buff2[baseIndex + i] = channel_buffer[i];
  }
...
*/
  ...
  unsigned int c = 0;
  unsigned int i = 0;
  for (; i < pixelCount; i++, c += 3) {
      leds.setPixel(i, buff2[c], buff2[c + 1], buff2[c + 2]);
  }
  leds.show();
}
...
void InitLeds(){
  ...
  leds.begin();
    ...
    for (unsigned int w=0; w<ledsPerStrip * 8; w=w+12) 
    {
...
void BlueBlinkieBlinkie() {
  ...
  for (int d =0;d<3;d++){
    for (int b=64; b>0;b=b-8){
        for (unsigned int i = 0; i < ledsPerStrip * 8; i++) {
